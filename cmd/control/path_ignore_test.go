...
func TestIgnorePath(t *testing.T) {
	...
	resp = postRequest("http://localhost:5692/api/path_ignore", "", "/")
	defer resp.Body.Close()
	expectResponse(t, resp, http.StatusUnsupportedMediaType, "Missing Content-Type header\n")

	// unsupported content type
	resp = postRequest("http://localhost:5692/api/path_ignore", "image/png", "/")
	defer resp.Body.Close()
	expectResponse(t, resp, http.StatusUnsupportedMediaType, "Unsupported Content-Type: image/png\n")

	// some strings
	resp = postRequest("http://localhost:5692/api/path_ignore", "text/plain", "/path1\n/path2\n")
	defer resp.Body.Close()
	expectResponse(t, resp, http.StatusOK, "`/path1` was added\n`/path2` was added\n")

	// duplicate strings
	resp = postRequest("http://localhost:5692/api/path_ignore", "text/plain", "/path3\n/path3\n")
	defer resp.Body.Close()
	expectResponse(t, resp, http.StatusBadRequest, "`/path3` was added\nPath is already ignored: /path3\n")

	// no paths provided
	resp = postRequest("http://localhost:5692/api/path_ignore", "text/plain", "")
	defer resp.Body.Close()
	expectResponse(t, resp, http.StatusBadRequest, "No paths to ignore were provided\n")
	...
	// get ignored paths
	resp = getRequest("http://localhost:5692/api/path_ignore")
	defer resp.Body.Close()
	expectResponse(t, resp, http.StatusOK, "/path1\n/path2\n/path3\n.*\n(.*)\n")

	// delete ignored paths
	resp = deleteRequest("http://localhost:5692/api/path_ignore")
	defer resp.Body.Close()
	...
	resp = putRequest("http://localhost:5692/api/path_ignore", "", "")
	defer resp.Body.Close()
