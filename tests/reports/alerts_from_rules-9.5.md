...
## Table of contents
   1. [Failed rules (25)](#failed-rules-25)
   1. [Unsuccessful rules with signals (25)](#unsuccessful-rules-with-signals-25)
   1. [Rules with no signals (4)](#rules-with-no-signals-4)
   1. [Rules with too few signals (29)](#rules-with-too-few-signals-29)
   1. [Rules with the correct signals (812)](#rules-with-the-correct-signals-812)
...
## Unsuccessful rules with signals (25)
...
Index: geneve-ut-0403  
Failure message(s):  
  got 1000 signals, expected 1680  
...
Index: geneve-ut-1036  
Failure message(s):  
  got 1000 signals, expected 1170  
...
## Rules with the correct signals (812)
...
container.security_context.privileged == true and container.id like "*"
```
...
sequence by container.id, user.id with maxspan=3s
  [file where host.os.type == "linux" and event.type == "creation" and container.id like "?*" and
...
registry where host.os.type == "windows" and event.type == "change" and
  ...
  /* Microsoft Cloud AppCompat SDB test registrations */
  not registry.path : "*\\AppCompatFlags\\Custom\\*\\{22221111-1111-1111-1111-111111111111}.sdb"
```
...
sequence by host.id with maxspan=30s
 ...
 [process where host.os.type == "macos" and event.type in ("start", "process_started") and process.name == "launchctl" and process.args in ("load", "bootstrap")] by process.parent.entity_id
```
...
process where host.os.type == "windows" and event.type == "start" and
  ...
  not user.id : "S-1-5-18" and
  not process.args : (
    "?:\\Program Files (x86)\\Microsoft Power BI Report Server\\*.mof",
    "?:\\Program Files (x86)\\Microsoft SQL Server Reporting Services\\*.mof",
    "?:\\Program Files (x86)\\Microsoft SQL Server\\*.mof",
    "?:\\Program Files\\Microsoft Policy Platform\\*.mof",
    "?:\\Program Files\\Microsoft Power BI Report Server\\*.mof",
    "?:\\Program Files\\Microsoft SQL Server Reporting Services\\*.mof",
    "?:\\Program Files\\Microsoft SQL Server\\*.mof",
    "?:\\Windows\\System32\\wbem\\com.hp.provider.*.mof",
    "?:\\Windows\\System32\\wbem\\CxAudioSvc_*.mof",
    "?:\\Windows\\System32\\wbem\\Framework\\root\\*.mof",
    "?:\\Windows\\System32\\wbem\\Veeam.Backup.*.mof",
    "?:\\Windows\\System32\\wbem\\ZTITatoo.mof",
    "?:\\Windows\\SysWOW64\\wbem\\Framework\\root\\*.mof"
  )
```
...
) and container.security_context.privileged == true and container.id like "*"
```
...
process.executable:(* and not (/bin/* or /nix/store/*/bin/sudo or /run/wrappers/wrappers*/sudo or /sbin/* or /usr/bin/* or /usr/sbin/* or "/usr/local/bin/pbrun")) and 
...
file.path in ("/usr/bin/sudo", "/bin/sudo") and not (
  ...
  process.executable in (
    ...
    "/var/ossec/bin/wazuh-syscheckd", "/usr/local/bin/defender", "/usr/sbin/dnf", "/usr/local/bin/starship", "/bin/dnf4",
    "/usr/sbin/yum-cron", "/bin/buildah", "/usr/bin/buildah"
  ) or
  file.Ext.original.extension == "dpkg-new" or
  file.Ext.original.name like "sudo;*" or
  process.executable : (
    ...
    "/var/lib/containers/storage/overlay/*/dockerd", "/usr/libexec/platform-python*"
  ) or
  process.executable == null or
  (process.name == "sed" and file.name : "sed*")
)
...
file.name == "notify_on_release" and container.id like "*"
```
...
```python
file where host.os.type == "linux" and event.type == "change" and event.action == "open" and
file.name == "release_agent" and container.id like "*"
```
  ...
  [process where host.os.type == "linux" and event.type == "start" and event.action == "exec" and process.name != null and
   ...
   user.id != "0" and not (
     ...
     process.parent.executable in (
       ...
       "/opt/SolarWinds/Agent/bin/Plugins/Discovery/SolarWinds.Agent.Discovery.Plugin",
        "/usr/sbin/pcoip-agent", "/usr/bin/cubic", "/usr/lib/cockpit/cockpit-ws",
       "/usr/lib/vmware/bin/vmware", "/usr/bin/vmware-fuseUI"
     ) or
     process.executable : ("/opt/dynatrace/*", "/tmp/newroot/*", "/opt/SolarWinds/Agent/*", "/snap/snapd/*/usr/lib/snapd/snap-confine") or
     process.executable in (
       "/bin/fgrep", "/usr/bin/sudo", "/usr/bin/pkexec", "/usr/lib/cockpit/cockpit-session", "/usr/sbin/suexec",
       "/usr/bin/at", "/usr/lib/pcoip-agent/pcoip-authentication-proxy"
     ) or
...
```python
file where host.os.type == "linux" and event.action in ("creation", "file_create_event", "rename", "file_rename_event")
and file.path like ("/etc/init.d/*", "/etc/init/*", "/etc/init", "/etc/inittab*") and
not (
  process.executable in (
    "/bin/dpkg", "/usr/bin/dpkg", "/bin/dockerd", "/usr/bin/dockerd", "/usr/sbin/dockerd", "/bin/microdnf",
    "/usr/bin/microdnf", "/bin/rpm", "/usr/bin/rpm", "/bin/snapd", "/usr/bin/snapd", "/bin/yum", "/usr/bin/yum",
...
) and source.as.number != 396982  
and not (
  user_agent.original : ("*ChatGPT-User*", "*GPTBot*", "*ClaudeBot*", "*anthropic-ai*", "*OAI-SearchBot*", "*perplexity.ai*", "*claude-searchbot*", "*anthropic.com*", "*google.com/bot.html*", "*TelegramBot*", "*openai.com/searchbot*", "*LinkedInBot*", "*x.ai/grokbot*", "*Discordbot*", "*Amazonbot*", "*amazonbot*", "*Applebot*", "*Slackbot*", "*Twitterbot*", "*Bytespider*")
  and http.response.status_code in (401, 403, 404, 429)
)
```
