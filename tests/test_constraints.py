...
class TestConstraints(tu.SeededTestCase, unittest.TestCase):
    ...
    def test_entity(self):
        ...
        self.assertEqual(entities[1], d.find_entity("process."))
        self.assertEqual(entities[1], d.find_entity("process.tty"))
        self.assertEqual(entities[3], d.find_entity("process.tty.char_device."))
        self.assertEqual(entities[3], d.find_entity("process.tty.char_device.major"))
        self.assertEqual(entities[7], d.find_entity("@timestamp"))

        msg = "'process.tty.thread'"
        with self.assertRaises(KeyError, msg=msg) as cm:
            self.assertEqual(None, d.find_entity("process.tty.thread."))
        self.assertEqual(msg, str(cm.exception))
